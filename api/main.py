...
# 배열을 한번에 받아서 오직 자스로만 페이징까지 해야할것같다.
# @app.get('/api/get_search_data')
# async def get_search_patient(path: str = '', search_str: str = ''):
#     search_str = f"%{search_str}%" # 검색 문자열 앞뒤에 % 추가
#     query = None
#     count = None
#     data=[]
...
#     switch_path = {
#         '/': "SELECT * FROM vital_record_now_view WHERE (pid LIKE :search_str OR name LIKE :search_str)",
#         '/detected': "SELECT * FROM now_view_sepsis WHERE (pid LIKE :search_str OR name LIKE :search_str)",
#     }
#     if path in switch_path:
#         query = text(switch_path[path])
#         # 검색 수행
